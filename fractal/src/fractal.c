...
//struct to represent all the places the header needs values (and what they are) in order to work
typedef struct {
...
//calculating the real part of the function (all of the three functions following are the same with minimal exceptions)
double funre(Z z, double *coef, int degree) {
    int k = 0; //counter that lowers the degree every loop for the next variable of the function
    double sum = 0; //sum for the real part if the imaginary
    complex s, upd; //introducing two complex variables 
    while (degree - k > 0) {
        s.real = 0; //making these two variables equal to zero(anything would work, they are temporary variables)
        s.imag = 0;
        upd.real = z.o.real; //giving these two update variables the variables passed through from main
        upd.imag = z.o.imag;
        if (coef[degree - k] == 0) { //avoiding variables that have a coefficient of zero as they have no impact on the sum
            k ++;
            continue;
        }
        for (int i = 2; i <= degree - k; i++) { //loop that starts from 2, as variables with degree 1 or less dont have anything to be multiplied by other than their coefficients
            s.real = remul(upd.real, upd.imag, z.o.real, z.o.imag); //finding the real part of the multiplication and storing it in a temporary variable
            s.imag = immul(upd.real, upd.imag, z.o.real, z.o.imag); //this time finding the imaginary part and storing. The reason I need the temps is because else the newly updated value would go into the immul() instead of the old one
        ...
        upd.real *= coef[degree - k]; //updating the real and imaginary values with their according coefficients
        upd.imag *= coef[degree - k];
        sum += upd.real; //adding the real part of the degree of the polynomial to the sum
        k++;
    }
    sum += coef[0]; //updating the sum with the last coefficient of the degree 0 which is equal to 1
    return sum; //returning the sum
}

//calculating the real part of the derivative function
double derfunre(Z z, double *coef, int degree) {
    double *dercoef = malloc(degree * sizeof(double)); //allocating the coefficients of the derivative and calculating them till line 77
    for (int i = degree; i >= 1; i--) {
        dercoef[i - 1] = i * coef[i];
    }
    int k = 0;
    degree--; //lessening the degree by 1 as derivatives are one degree lower than the functions they derived from
    double sum = 0; //from here on put in this function, the code is identical to the last function with the exception that instead of the coef[] array, dercoef[] was used
    complex s, upd;
    while (degree - k > 0) {
        s.real = 0;
        s.imag = 0;
...
//calculating the imaginary part of the function (the code is the same as the funre function but now sum calculates the imaginary part and the last coefficient isn't added as it's value is always real)
...
//calculating the imaginary part of the derivative function (same code as derfunre except sum is summing up the imaginary part of the derivative and the last coefficient is not calculated since it is always a real number)
...
void check_colors(Z z, unsigned char **pixel_array[], int height, int width, int *roots, int *num_roots, unsigned char *colors[], int degree) {
    z.n.real *= 100; //multiplying both variables by 100 to bring the digit(s) and the first two decimal digits after the decimal point
    z.n.imag *= 100;
    if ((*num_roots) == 0) { //this if statement will only be true on the first entry
        (*num_roots) ++; //cool way to permantently update a variable, works by putting the address of the variable in the function
        roots[0] = (int)z.n.real; //saving the integer real and integer imaginary parts of the root in two separate cells
        roots[1] = (int)z.n.imag;
        pixel_array[height][width][0] = colors[0][0]; //asserting the first color in the pixel array
        pixel_array[height][width][1] = colors[0][1];
        pixel_array[height][width][2] = colors[0][2];
    } else {
        for (int i = 0; i < ((*num_roots) * 2); i += 2) { //a for loop that scans every root (mulitplication and step by 2 is done because the parts of the roots are essentially separate due to line 169-170)
            if ((int)z.n.real == roots[i] && (int)z.n.imag == roots[i + 1]) { //if the real and imaginary integer parts inserted have been found already, then the pixel array gets updated
                pixel_array[height][width][0] = colors[i/2][0]; //division of i by 2 is done because the step goes up by 2 due to the separate roots so the adjacent color of the rootis at it's half
...
int main(int argc, char **argv) {
    if (argc != 2 && argc != 4) { //accepting two inputs due to the nature of the assignment, either simple newton fractal or option for it's visualization as well
        fprintf(stderr, "Wrong input number. Normal input is of type ./fractal filename or ./fractal filename -g output.bmp\n");
        exit(1);
    } 
    FILE *comp = fopen(argv[1], "r"); //opening the first argument (file) for read only, where the degree, coefficients, number ranges and the step will be taken from
    if (comp == NULL) { //checking if the file is empty
        fprintf(stderr, "file could not be opened\n");
        exit(1);
    }
    int degree; //introducing a variable for the degree of the polynomials
    if (!(fscanf(comp, "%d", &degree))) exit(1); //scanning the degree off of the input
    if (degree > 10) exit(1);
    double *coef = malloc((degree + 1) * sizeof(double)); //allocating memory for the coefficients
    for (int i = 0; i <= degree; i ++) {
        if(!(fscanf(comp, "%lf", &coef[i]))) exit(1); //scanning each coefficient till the degree
    }
    ...
    double step; //creating a variable for the step of each loop (the resolution of the image)
    ...
    for (int i = 0; i < height; i ++) {
        for (int j = 0; j < width; j ++) {
            for (int a = 0; a < 3; a++) pixel_array[i][j][a] = 0; //turning every cell into 0 as it would otherwise possibly be used uninitialized later
        }
    }
    unsigned char **colors = malloc(degree * sizeof(unsigned char*)); //allocating memory for the colors
        if (colors == NULL) exit(1);
        for (int i = 0; i < degree; i++) {
            colors[i] = malloc(3 * sizeof(unsigned char));
        }
    //from line 238 to 298, manually adding colors to the array, that will be later passed on to the pixel array
    int k = 0;
    if (k + 1 <= degree) {
        colors[k][0] = 0;
        colors[k][1] = 255;
    ...
    Z z; //creating a "double" complex value for the old and new variable where each of them have their own real and imaginary parts
    ...
    for (double i = min.real; i < max.real; i+=step) {  
        ...
        for (double j = min.imag; j < max.imag; j+=step) {
            ...
            z.n.imag = j;
            do {
                z.o.real = z.n.real; //passing the new variables to the old ones as they are about to change
                z.o.imag = z.n.imag;
                if((derfunre(z, coef, degree) == 0 && derfunim(z, coef, degree) == 0)) break; //checking if the derivates are zero (division by 0)
                z.n.real -= rediv(funre(z, coef, degree), funim(z, coef, degree), derfunre(z, coef, degree), derfunim(z, coef, degree)); //implementing the newton raphson method for the real and the imaginary part
            ...
            count_rep++;
            if (count_rep == 10e6) break; //if the repetitons reach the 10^6 then the loops stop
            if (count_cur <= 1000) {
                if((derfunre(z, coef, degree) == 0 && derfunim(z, coef, degree) == 0)) { //if the derivatives are zero then no root could be calculated
                    printf("nan\t");
                    if (argc == 4) { //if the user chose 4 arguments then assign the color black to the array
            ...
            } else { //if count of repetitions then stop the loop and print incomplete
                printf("incomplete\t");
                if (argc == 4) { //if visualization is picked, assign the color white to the array
    ...
    if (argc == 4) { //from here till the end of the code
        ...
        for (int i = 0; i < 54; i++) header[i] = 0; //assigning each header cell to 0
        header[0] = 'B'; //first two bytes
        header[1] = 'M';
        BMPhead info; //creating a struct variable for all the different points of the header
        info.offset = 54; //offset ends after 54 bytes from the starts so that's the value im assigning it
        header[OFFSET] = info.offset; //inserting the offset into the array
        info.headsize = 40; //adjusting the header size that marks the start of the pixel array
        ...
        header[WIDTH] = info.width & 0xff; //since width and height are integers, we assign the first byte (the least significant one) to the header at the position, then move up 8 bits to the next byte etc. This helps with bmp pictures whose height and width exceed 255
        ...
        info.bits = 24; //this is the number of bits per pixer and since 8 bits = 1 byte then 24 / 8 = 3 bytes in each pixel
        header[BITSPERPIX] = *(unsigned char *)&info.bits;
        info.xppm = 96; //this is the standard value of pixels per meter in windows, both for the x and y axis
        ...
        hex = 0;
        unsigned char *final = malloc((3 * height * width + height * pad) * sizeof(unsigned char)); //creating an array the size of PIXBYTES to store each pixel and it's bytes in 1 dimension
        ...
        for (int i = height - 1; i >= 0; i --) { //in little endian the height starts from the bottom left instead of top left
            for (int j = 0; j < width; j ++) {
                if (pad != 0 && flag == 1) { //if there is padding and the boolean value is TRUE then from line 442 - 456 we adjust the padding and update the variables accordingly
                    if (pad == 1) {
        ...
        read = fwrite(final, sizeof(unsigned char), 3 * height * width + height * pad, out); //using fwrite to put all the bytes in the pixel array of the bmp for the final image
