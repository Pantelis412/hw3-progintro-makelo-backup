...
// we create the nodes of the tree that are free around the one we are currently
// processing and add them in the back of the queue
void add_node(tree *p, int U, int R, int D, int L, int row, int collumn,
              char **visited, queue **tail) {
  tree *previous;
  previous = p;
  if (U == 1) {
    p = p->up;
    ...
    p->row = row - 1;
    p->collumn = collumn;
    p->key = visited[p->row][p->collumn];  // first we save what was on the tile
                                           // (trash is '2')
    visited[p->row][p->collumn] =
        '1';  // then we fill this tile of visited array with '1' to be able to
...
void find_path(char **visited, int RowZ, int ColZ) {
  ...
  int U, R, D, L;
  tree *navigator = NULL;
  // check for the trash in the front of the queue, add the surrounding nodes of
  // the tree in the queue and pop the current node form the queue
  while (head != NULL) {
    if (head->pointer->key ==
        '2') {  // we check if the head of the queue is the tile with the trash.
      navigator = head->pointer;
      break;
    }
    U = R = D = L = 0;  // these letters symbolizes the directions
    int adjx;  // these are temporary variables that respresent the tiles around
               // the one we currently are
    int adjy;
    for (int i = 0; i < 4; i++) {
      adjx = head->pointer->row;  // reinitialize in every loop
      adjy = head->pointer->collumn;
      adjx = head->pointer->row +
             DirR[i];  // checking the tiles up, down, left and right according
                       // with these 2 arrays
      adjy = head->pointer->collumn + DirC[i];
      if (existence(visited, adjx,
                    adjy)) {  // if the tile is in bounds and is free(not wall)
        if (i == 0) U = 1;
        if (i == 1) R = 1;
        if (i == 2) D = 1;
        if (i == 3) L = 1;
    ...
    add_node(head->pointer, U, R, D, L, head->pointer->row,
             head->pointer->collumn, visited, &tail);
    temp_head = head->next;  // free the head of the queue(the node we currently
                             // processed)
    free(head);
    head = temp_head;
  }
  ...
  if (navigator == NULL)
    ...
    while (navigator != root) {
      ...
      // we calculate each step of the backtrack based on the positions of the
      // current and the previous nodes of the tree
...
int main(void) {
  ...
  if (dim > 10000)
    exit(1);  // checking if the dimensions exceed the allowed boundaries
  if (poszoo_x >= dim || poszoo_y >= dim || trash_x >= dim || trash_y >= dim)
    exit(1);  // checking if the position of the machine or the trash is out of
              // bounds
  ...
  if (room[poszoo_x][poszoo_y] == '1' || room[trash_x][trash_y] == '1')
    exit(1);  // checking if the machine or the trash is on top of an obstacle
  ...
  char **visited =
      malloc(dim * sizeof(char *));  // introducing a 2-dimensional array that
  ...
  for (int i = 0; i < dim;
       i++) {  // freeing the two-dimensional array of the room
    free(visited[i]);
  }
  free(visited);
  for (int i = 0; i < dim;
